...
abstract class PlayerIO(coordinator: PlayerCoordinator) extends Listener {
  ...
  def prelogin(event: AsyncPlayerPreLoginEvent): Unit ={
    val player = coordinator(event.getUniqueId)
    if(player.banned){
      event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_BANNED, Punishment.reason(player._ban.get))
      coordinator.unload(event.getUniqueId)
    }
  }

  @EventHandler def login(event: PlayerLoginEvent): Unit = {
    if(!coordinator.has(event.getPlayer.getUniqueId)) coordinator(event.getPlayer.getUniqueId)
  }

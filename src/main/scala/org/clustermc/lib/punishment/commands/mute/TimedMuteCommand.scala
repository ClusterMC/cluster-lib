...
import org.clustermc.lib.utils.messages.vals.PunishmentMsg.{punishmentErrorInvalidDuration, punishmentMuteTempMuter}
...
object TimedMuteCommand extends PunishmentCommand{
  ...
  override def punish(punisher: Player, punished: Player, reason: String, duration: String): Unit = {
    ...
    PunishmentManager.tempmute(time, punisher.getName, punisher.getUniqueId, punished.getUniqueId, reason)
    punisher.sendMessage(punishmentMuteTempMuter(punished.getName, duration, reason).get)
  }
